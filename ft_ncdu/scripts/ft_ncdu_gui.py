...
class App(tk.Tk):
    ...
    def scan_path(self, path):
        ...
        self.scanner = Scanner(self.current_path, self.on_scan_complete, self.on_scan_update)
        self.scanner.start()
    ...
    def action_open_editor(self):
        ...
        if not editor:
            ...
            return
        try:
            subprocess.Popen([editor, str(node.path)])
    ...
    def action_open_terminal(self):
        ...
        if not shell:
            ...
            return
        try:
            subprocess.Popen([shell], cwd=str(self.current_path))
        except FileNotFoundError:
            messagebox.showerror("Error", f"Shell '{shell}' not found.")
    ...
    def action_custom_command(self):
        ...
        if cmd:
            ...
            subprocess.Popen(full_cmd, shell=True, cwd=str(self.current_path))

    def action_empty_trash(self):
        if messagebox.askyesno("Empty Trash", "Empty the desktop trash bins?"):
            subprocess.run('rm -rf ~/.local/share/Trash/* ~/.Trash/* 2>/dev/null', shell=True)
...
"""
    ...
    def action_docker_prune(self):
        if messagebox.askyesno("Docker Prune", "Run 'docker system prune -a --volumes -f'?"):
            subprocess.Popen("docker system prune -a --volumes -f", shell=True)
